    ...
    namespace TETRA_DMO
    {
            ...
            : constraint_c(constraint), polynomials_c(polynomials)
        {
            ...
            for (std::size_t i = 0; i < polynomials_c.size(); i++)
            {
                ...
                std::cout << "  Polynomial " << i + 1 << ": " << (polynomials_c[i]) 
                  << " (" << polynomials_c[i] << ")" << std::endl;
            }
            std::cout <<"constraint push: " << constraint_c;
            InitializeOutputs();
        }
        ...
        std::string ViterbiCodec::Decode(const std::string& bits) const
        {
            ...
            // Error: constrain_c here has been put into 1, which insist that the codec hasn't been created: conflict. 
            Trellis trellis;
            // std::cout <<"constraint is" << constraint_c << "\n";
