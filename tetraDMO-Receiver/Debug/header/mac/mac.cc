...
void Mac::serviceLowerMac(std::vector<uint8_t> data, int burstType)
{
    ...
    else if (burstType == DSB)                                               // DMO Sync Burst
    {
        ...
        {
            bkn1 = vectorAppend(vectorExtract(bkn1, 0, 60), vectorExtract(bkn2, 0, 124)); // reconstruct block to BKN1
            serviceUpperMac(bkn1, DSCH_SH);                                    // merge bkn1 60 bits + bkn2 124 bits
        }
        else    // for debug purpose - display message if CRC errors found
        {
            printf("DSB Burst CRC check errors found bkn1/bkn2: %u/%u, TN/FN/MN: %u/%u/%u. \n\n",
            checkCrc16Ccitt(bkn1, 76), checkCrc16Ccitt(bkn2, 140), getTime().tn, getTime().fn, getTime().mn);
        }
    }
    else if (burstType == DNB)      // carry TCH traffic channel (data/speech) or SCH/F full slot signalling channel 
    {

        // BKN1 + BKN2
        bkn1 = vectorAppend(vectorExtract(data, 14, 216), vectorExtract(data, 252, 216)); // reconstruct block to BKN1
        bkn1 = descramble(bkn1, 432, m_tetraCell->getScramblingCode());                   // descramble

        if (true)                              // assume ALWAYS traffic mode - unless SCH/F channel - TODO, no handle yet
        {
            serviceUpperMac(bkn1, DTCH_S);     // full frame (432 bits) is sent directly to User plane
        }
    }

    else if (burstType == DNB_SF)      //ONLY carry STCH stealing channel
    {
        bool bkn1ValidFlag = false;
        bool bkn2ValidFlag = false;
        ...
        {
            serviceUpperMac(vectorExtract(data, 282, 216), DTCH_S);      // SF frame: data length only 216 bits
        }
...
void Mac::serviceUpperMac(Pdu data, MacLogicalChannel macLogicalChannel)
{
    ...
    do
    {
        ...
        case DSCH_SH:                                                           // DMAC-SYNC PDU in SCH/SH - stop after processing
            txt = "dsch/sh";
            tmSdu = pduProcessDmacSync(pdu);
            break;

        case DSTCH:
            m_log->print(LogLevel::NONE, "DMO STCH : TN/FN/MN = %2d/%2d/%2d    dl_usage_marker=%d, encr=%u\n", m_tetraTime.tn, m_tetraTime.fn, m_tetraTime.mn, m_macState.downlinkUsageMarker, m_usageMarkerEncryptionMode[m_macState.downlinkUsageMarker]);
            txt = "  dstch";
            // process DMAC DATA PDU for STCH channel
            tmSdu = pduProcessDmacData(pdu);
            break;

        case DTCH_S:
            m_log->print(LogLevel::NONE, "DMO TCH_S: TN/FN/MN = %2d/%2d/%2d    dl_usage_marker=%d, encr=%u\n", m_tetraTime.tn, m_tetraTime.fn, m_tetraTime.mn, m_macState.downlinkUsageMarker, m_usageMarkerEncryptionMode[m_macState.downlinkUsageMarker]);
            txt = " dtch_s";
            /* calling same uPlane service - overloaded function for DMO DMAC processing
            Because there is only 01 voice call in DMO channel, ASSUME Clear-No encryption. Traffic voice belongs to only 01 call
            Only need m_tetraTime to check whether any missing frame, in order to replace with dummy frame.
            */
            m_uPlane->service(pdu, DTCH_S, m_tetraTime);
            break;
...
Pdu Mac::pduProcessSync(Pdu pdu)
{
    ...
    if (pdu.size() >= MIN_SIZE)
    {
        ...
        {
            m_log->print(LogLevel::NONE, "BSCH        : TN/FN/MN = %2u/%2u/%2u  MAC-SYNC        Scrambling Code=%8X  LA=%3d  MCC/MNC = %3u/ %3u  Freq= %10.6f MHz  burst=%u\n",
                   m_tetraTime.tn,
                   m_tetraTime.fn,
                   m_tetraTime.mn,
                   m_tetraCell->getScramblingCode(),
...
/* Process DMAC SYNC*/
Pdu Mac::pduProcessDmacSync(Pdu pdu)
{
    m_log->print(LogLevel::HIGH, "DEBUG ::%-44s - pdu = %s\n", "mac_pdu_process_DmacSync", pdu.toString().c_str());
    ...
    if (pdu.size() >= MIN_SIZE)
    {
        ...
        // for debug - print parameters 
        m_log->print(LogLevel::NONE, "DSCH_SH  : TN/FN/MN = %2u/%2u/%2u    DMAC-SYNC Scrambling Code=%8X  MCC/MNC = %3u/ %3u  burst=%u\n",
                m_tetraTime.tn,
                m_tetraTime.fn,
                m_tetraTime.mn,
                m_tetraCell->getScramblingCode(),
        ...
        m_log->print(LogLevel::NONE, "System Code  = %u  SYNC PDU type = %2u  Communication type= %2u  Fill bit indication= %u Fragmentation flag=%u\n",
                systemCode, syncPduType, commType, fillBitInd, fragFlag);

        m_log->print(LogLevel::NONE, "Frame Cntdown= %2u  Destination address= %u/%4u  Source address= %u/%4u Message type= %2X\n\n",
                frameCntDn, destinAddrType, destinAddress, sourceAddrType, sourceAddress, messageType);
    }
    else
    {
        m_report->add("invalid pdu size", (uint64_t)pdu.size());
...
/* Process DMAC DATA PDU*/
Pdu Mac::pduProcessDmacData(Pdu pdu)
{
    m_log->print(LogLevel::HIGH, "DEBUG ::%-44s - pdu = %s\n", "mac_pdu_process_DmacData", pdu.toString().c_str());

    Pdu sdu;
    m_sfStolenFlag = false;                     // set second half slot stolen = False
    ...
    if (pdu.size() >= MIN_SIZE)
    {
        ...
        m_sfStolenFlag = sfStolenFlag;                      // raise flag for serviceUpperMac to process bkn2
        sdu = Pdu(pdu, pos, MIN_SIZE - pos);
        ...
        // for debug - print parameters 
        m_log->print(LogLevel::NONE, "DSTCH    : TN/FN/MN = %2u/%2u/%2u    DMAC-DATA Scrambling Code=%8X  MCC/MNC = %3u/%3u  Burst type=%u\n",
                m_tetraTime.tn,
                m_tetraTime.fn,
                m_tetraTime.mn,
                m_tetraCell->getScramblingCode(),
        ...
        m_log->print(LogLevel::NONE, "MAC PDU type = %2u  Fill bit indication= %u Second H/S stolen flag= %u  Fragmentation flag=%u  Null PDU flag=%u\n",
                dmacPduType, fillBitInd, sfStolenFlag, fragFlag, nullPduFlag);

        m_log->print(LogLevel::NONE, "Frame Cntdown= %2u  Destination addr= %u/%4u  Source addr type= %u/%u MCC/MNC=%u/%2u  Message type= %2X\n\n",
                frameCntDn, destinAddrType, destinAddress, sourceAddrType,sourceAddress, mnIdentity>>14, (mnIdentity & 0x3fff), messageType);
