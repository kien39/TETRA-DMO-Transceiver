...
MacEncoderLib = cdll.LoadLibrary('/home/ctn008/tetraDMO-Transmitter/MacEncoderLib.so') # class level loading lib
...
fin ='/home/ctn008/tetraDMO-Receiver/codec/tetraDMO_s36kBit_uplane.uint8'
in_data = np.fromfile(fin, dtype = 'uint8')
out_data = np.array([0]*len(in_data)*2, dtype = 'uint8')

res = serviceMacDecode(in_data, len(in_data), out_data)
