...
fileNchan = '/home/ctn008/tetraDMO-Receiver/codec/audio_src_01GENPF.chan'
...
while raw_ptr + 2*L_FRAME <= len(sig_raw):
    ...
    Vocod_array[  0:137] = serial[1:138]

    new_speech = np.array(sig_raw[raw_ptr+L_FRAME:raw_ptr+2*L_FRAME])
    Pre_Process(new_speech, L_FRAME)
    Copy_Speech_Frame(new_speech)    # copy to c global 'new_speech' pointer
    Coder_Tetra(ana, syn)
    ...
    sig_cod[cod_ptr+SERIAL_SIZE:cod_ptr+2*SERIAL_SIZE] = serial
    Vocod_array[137:274] = serial[1:138]

    raw_ptr += 2*L_FRAME
    cod_ptr += 2*SERIAL_SIZE
    frame +=2
    print("frame= %d" %frame)
